...
class BCEDiceLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor, leaf_mask: torch.Tensor | None = None) -> torch.Tensor:
        """
        The preferred pipeline is to remove the background at the dataset level
        using the leaf mask before passing the image to the model. In that setup,
        the loss remains the standard whole-image BCE + Dice objective.
